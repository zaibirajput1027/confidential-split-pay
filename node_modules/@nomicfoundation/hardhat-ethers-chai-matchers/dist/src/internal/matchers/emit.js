...
async function waitForPendingTransaction(tx, provider) {
    ...
    if (hash === null) {
        throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.INVALID_TRANSACTION, { transaction: JSON.stringify(tx) });
    }
    ...
    Assertion.addMethod(EMIT_MATCHER, function (contract, eventName, ...args) {
        ...
        const onSuccess = (receipt) => {
            ...
            if (typeof contractAddress !== "string") {
                throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.CONTRACT_TARGET_MUST_BE_A_STRING);
            }
            if (args.length > 0) {
                throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.EMIT_EXPECTS_TWO_ARGUMENTS);
            }
        ...
        const derivedPromise = promise.then(() => {
            ...
            if (contract.runner === null || contract.runner.provider === null) {
                throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.CONTRACT_RUNNER_PROVIDER_NOT_NULL);
            }
