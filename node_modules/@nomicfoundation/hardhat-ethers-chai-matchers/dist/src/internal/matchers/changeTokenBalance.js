...
function validateInput(obj, token, accounts, balanceChanges) {
    try {
        ...
        if (Array.isArray(balanceChanges) &&
            ...
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.ACCOUNTS_NUMBER_DIFFERENT_FROM_BALANCE_CHANGES, {
...
function checkToken(token, method) {
    if (!isObject(token) || token === null || !("interface" in token)) {
        throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.FIRST_ARGUMENT_MUST_BE_A_CONTRACT_INSTANCE, {
            method,
        });
    }
