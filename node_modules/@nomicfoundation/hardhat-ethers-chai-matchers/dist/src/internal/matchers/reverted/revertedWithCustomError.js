...
function validateInput(obj, contract, expectedCustomErrorName, args) {
    try {
        ...
        if (typeof contract === "string" || contract?.interface === undefined) {
            ...
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.FIRST_ARGUMENT_MUST_BE_A_CONTRACT);
        }
        // validate custom error name
        if (typeof expectedCustomErrorName !== "string") {
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.STRING_EXPECTED_AS_CUSTOM_ERROR_NAME);
        }
        ...
        if (expectedCustomError === null) {
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.CONTRACT_DOES_NOT_HAVE_CUSTOM_ERROR, {
                customErrorName: expectedCustomErrorName,
            });
        }
        if (args.length > 0) {
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.REVERT_INVALID_ARGUMENTS_LENGTH);
        }
...
export async function revertedWithCustomErrorWithArgs(context, Assertion, _chaiUtils, expectedArgs, ssfi) {
    ...
    if (customErrorAssertionData === undefined) {
        throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.WITH_ARGS_FORBIDDEN);
    }
