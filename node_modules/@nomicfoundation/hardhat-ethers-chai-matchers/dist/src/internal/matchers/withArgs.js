...
function validateInput(chaiUtils) {
    try {
        if (Boolean(this.__flags.negate)) {
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.WITH_ARGS_CANNOT_BE_COMBINED_WITH_NOT);
        }
        ...
        if (!emitCalled && !revertedWithCustomErrorCalled) {
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.WITH_ARGS_WRONG_COMBINATION);
        }
        if (emitCalled && revertedWithCustomErrorCalled) {
            throw new HardhatError(HardhatError.ERRORS.CHAI_MATCHERS.GENERAL.WITH_ARGS_COMBINED_WITH_INCOMPATIBLE_ASSERTIONS);
        }
        return { emitCalled };
    }
    catch (e) {
