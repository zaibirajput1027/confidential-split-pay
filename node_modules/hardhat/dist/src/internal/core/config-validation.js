...
export function validateTasksConfig(tasks, path = []) {
    const validationErrors = [];
    for (const [index, task] of tasks.entries()) {
        ...
        switch (task.type) {
            ...
            case TaskDefinitionType.NEW_TASK: {
                validationErrors.push(...validateNewTask(task, [...path, "tasks", index]));
                break;
            }
            case TaskDefinitionType.TASK_OVERRIDE: {
                validationErrors.push(...validateTaskOverride(task, [...path, "tasks", index]));
                break;
            }
        }
    }
...
export function validateNewTask(task, path) {
    const validationErrors = [];
    if (!Array.isArray(task.id) ||
        !task.id.every((id) => typeof id === "string")) {
        validationErrors.push({
...
export function validateTaskOverride(task, path) {
    const validationErrors = [];
    if (!Array.isArray(task.id) ||
        !task.id.every((id) => typeof id === "string")) {
        validationErrors.push({
    ...
    for (const [index, plugin] of plugins.entries()) {
        ...
        if (plugin.tasks !== undefined) {
            if (Array.isArray(plugin.tasks)) {
                validationErrors.push(...validateTasksConfig(plugin.tasks, [...path, "plugins", index]));
            }
            else {
                validationErrors.push({
